...
public class JukeboxScreenHandler extends ScreenHandler {
    ...
    private final Inventory discInventory;

    private Boolean clientShuffleModeOverride = null;

    public JukeboxScreenHandler(int syncId, PlayerInventory playerInv, BlockPos pos, JukeboxBlockEntity jukebox) {
        ...
        //?} else {
        /*this.discInventory = new SimpleInventory(1) {
            @Override
            public ItemStack getStack(int slot) {
                return jukebox.getStack();
            }
            @Override
            public void setStack(int slot, ItemStack stack) {
                jukebox.setStack(stack);
            }
        };
        *///?}
        ...
        if (jukebox instanceof JukeboxSettingsProvider provider) {
            ...
            this.queueInventory = new SimpleInventory(ShuffleQueue.QUEUE_SIZE);
        }
    ...
    //? if >=1.20.5 {
    public JukeboxScreenHandler(int syncId, PlayerInventory playerInv, BlockPos pos) {
        super(SoundscapeScreenHandlers.JUKEBOX_SCREEN_HANDLER, syncId);
        this.pos = pos;
        this.jukebox = null;
        this.propertyDelegate = new ArrayPropertyDelegate(7);
        this.addProperties(propertyDelegate);
        this.discInventory = new SimpleInventory(1);
        this.queueInventory = new SimpleInventory(ShuffleQueue.QUEUE_SIZE);
    ...
    //?} else {
    /*public JukeboxScreenHandler(int syncId, PlayerInventory playerInv, PacketByteBuf buf) {
        super(SoundscapeScreenHandlers.JUKEBOX_SCREEN_HANDLER, syncId);
    ...
    *///?}
    ...
    public ItemStack getDiscStack() {
        if (!slots.isEmpty()) {
            return slots.get(DISC_SLOT).getStack();
        }
    ...
    public ItemStack quickMove(PlayerEntity player, int slotIndex) {
        ...
        if (slot != null && slot.hasStack()) {
            ...
            if (slotIndex >= QUEUE_SLOT_START && slotIndex < QUEUE_SLOT_START + QUEUE_SLOT_COUNT) {
                if (!this.insertItem(originalStack, PLAYER_INV_START, HOTBAR_START + HOTBAR_COUNT, true)) {
                    return ItemStack.EMPTY;
                }
            }
            else if (slotIndex >= PLAYER_INV_START) {
                if (ShuffleQueue.isDisc(originalStack)) {
                    if (!this.insertItem(originalStack, QUEUE_SLOT_START, QUEUE_SLOT_START + QUEUE_SLOT_COUNT, false)) {
                        return ItemStack.EMPTY;
                    }
                } else {
                    if (slotIndex < HOTBAR_START) {
                        if (!this.insertItem(originalStack, HOTBAR_START, HOTBAR_START + HOTBAR_COUNT, false)) {
                            return ItemStack.EMPTY;
                        }
                    } else {
                        if (!this.insertItem(originalStack, PLAYER_INV_START, HOTBAR_START, false)) {
                            return ItemStack.EMPTY;
                        }
                    }
                }
    ...
    private static class JukeboxPropertyDelegate implements PropertyDelegate {
        private final JukeboxBlockEntity jukebox;
