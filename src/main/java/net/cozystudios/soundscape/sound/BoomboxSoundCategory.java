...
import net.cozystudios.soundscape.mixin.client.SoundCategoryAccessor;
...
public class BoomboxSoundCategory {
    ...
    public static SoundCategory BOOMBOX = SoundCategory.RECORDS;
    ...
    public static void register() {
        try {
            ...
            for (Field f : SoundCategory.class.getDeclaredFields()) {
                if (Modifier.isStatic(f.getModifiers()) && f.getType() == SoundCategory[].class) {
                    valuesField = f;
                    break;
                }
            }
            if (valuesField == null) {
                Soundscape.LOGGER.warn("Could not locate SoundCategory values array; falling back to RECORDS");
                return;
            }
            valuesField.setAccessible(true);
            SoundCategory[] existing = (SoundCategory[]) valuesField.get(null);

            SoundCategory boombox = SoundCategoryAccessor.soundscape$init(
                    "BOOMBOX", existing.length, "boombox");

            SoundCategory[] combined = new SoundCategory[existing.length + 1];
    ...
    public static float getEffectiveVolume() {
        ...
        try {
            return mc.options.getSoundVolume(BOOMBOX);
        } catch (Throwable t) {
            return 1.0f;
        }
    }
    ...
    private static void ensureMapInjection(GameOptions options) {
        if (mapInjectionAttempted) return;
        mapInjectionAttempted = true;
        try {
            Map<SoundCategory, Object> soundMap = null;
            for (Field f : options.getClass().getDeclaredFields()) {
                ...
                if (firstKey instanceof SoundCategory) {
                    soundMap = (Map<SoundCategory, Object>) v;
                    break;
                }
            }
            if (soundMap == null) {
                Soundscape.LOGGER.warn("Could not locate sound volume map on GameOptions");
                return;
            }
            if (soundMap.containsKey(BOOMBOX)) return;

            Object recordsOption = soundMap.get(SoundCategory.RECORDS);
            ...
            for (Method m : options.getClass().getDeclaredMethods()) {
                ...
                if (params[0] != String.class || params[1] != SoundCategory.class) continue;
